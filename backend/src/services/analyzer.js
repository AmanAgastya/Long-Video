...
const MIN_CLIP_SECONDS = Number(process.env.MIN_CLIP_SECONDS || 140);
// Was 60, then 130, then 120 - clips were consistently coming back short
// because nothing in the prompt told the model it was *allowed* to run
// long; "self-contained moment" on its own reads as "keep it tight", so it
// defaulted to trimming every clip down to the shortest span that still
// made sense rather than using the room it had. The ceiling is paired with
// an explicit rule below telling the model to size each clip to how long
// the actual moment runs (up to this ceiling) instead of trimming for its
// own sake - so a quick one-liner still comes back short, but a longer
...
${clipTranscript}`;
  ...
  //
  // This was missing the same fix already applied to analyzeTranscriptChunk
  // above: ANALYSIS_MODEL (openai/gpt-oss-120b) is a reasoning model that
  // spends an unbounded, invisible chunk of max_tokens on hidden reasoning
  // before it writes the actual JSON answer. With max_tokens: 180 and no
  // reasoning_effort cap, the hidden reasoning alone was eating the entire
  // budget, so the response got cut off before any JSON was written and
  // `JSON.parse(completion.choices[0].message.content || "{}")` below threw
  // on empty/invalid content - surfaced to the user as "Failed to
  // regenerate caption." reasoning_effort: "low" leaves the actual JSON
  // output budget intact, and max_tokens is raised to give it real headroom.
  const completion = await retryAnalysisRequest(async () =>
